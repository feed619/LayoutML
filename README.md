...
LayoutML(layout markup library) — это библиотека которая позволяет описывать структуру веб-страниц, стили и компоненты без шаблонизаторов, напрямую через код, превращая Python в язык разметки для web-интерфейсов.
